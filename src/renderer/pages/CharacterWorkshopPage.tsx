...
import { db, ai, onAIProgress } from '../lib/bridge';
...
const CharacterWorkshopPage: React.FC = () => {
  ...
  const createCharacter = async () => {
    ...
    setProgressMsg(`✅ 角色「${char.name}」已创建`);
    ...
    <div className="animate-fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          />
          <span style={{ color: 'var(--text-muted)', fontSize: 13 }}>{characters.length} 个角色 · 已持久化</span>
        </div>
        ...
        <div className="animate-fade-in" style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 16 }}>
            ...
            <div style={{ gridColumn: '1/-1' }}>
              ...
              <textarea value={newChar.personality} onChange={(e) => setNewChar({ ...newChar, personality: e.target.value })}
            ...
            <div key={char.id} className="animate-fade-in" style={{
              ...
              {char.scriptsUsed.length > 0 && (
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>
                  📖 参演 {char.scriptsUsed.length} 部短剧
                </div>
              )}
